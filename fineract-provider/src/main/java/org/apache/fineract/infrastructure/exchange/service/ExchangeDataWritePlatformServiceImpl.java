...
public class ExchangeDataWritePlatformServiceImpl implements ExchangeDataWritePlatformService {
    ...
    public ExchangeDataWritePlatformServiceImpl(final PlatformSecurityContext context,
                                                ...
                                                final FromJsonHelper fromJsonHelper, final ObjectMapper objectMapper) {
    ...
    public CommandProcessingResult saveBSEData(Long groupId, JsonCommand command) {
        ...
        String requestBody = String.format("{'memberCode': %s, 'loginid': %s, 'password': %s, 'ibbsid': %s}", memberId, loginId, password, userName);
...
//            String message = this.fromJsonHelper.extractStringNamed("message", jsonElement);
            ...
            String ipoRequestBody = String.format("{'scriptId': %s, 'applicationno': %s, 'category': %s, 'applicantname': %s" +
                    "'depository': %s, 'dpid': %s, 'clientbenfid': %s, 'chequereceivedflag': %s, 'chequeamount': %s, 'panno': %s" +
                    "'bankname': %s, 'location': %s, 'accountnumber_upiid': %s, 'ifsccode': %s, 'referenceno': %s, 'asba_upiid': %s}");
            ...
            if (exchangeIQResponse.getErrorCode() != null) {
                new ExchangeIPORequestException(errorCode, errorMessage, applicationNo, applicantName);
            }
...
//                .build();
        ...
        try (CloseableHttpClient client = HttpClients.createDefault()) {
            ...
            ExchangeLoginResponseData response = client.execute(request, httpResponse ->
                    objectMapper.readValue(httpResponse.getEntity().getContent(), ExchangeLoginResponseData.class));
    ...
    public ExchangeIQResponse ipoOrder(String baseAPIURL, String requestBody, String tokenAPI) throws IOException {
        ...
        try (CloseableHttpClient client = HttpClients.createDefault()) {
            ...
            ExchangeIQResponse response = client.execute(request, httpResponse ->
                    objectMapper.readValue(httpResponse.getEntity().getContent(), ExchangeIQResponse.class));
